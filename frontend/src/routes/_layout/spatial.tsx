...
const getClusterIcon = (color: string) => createDivIcon(`<div class="w-4 h-4 rounded-full border-2 border-black shadow-[0_0_15px_${color}]" style="background-color: ${color};"></div>`, 16)

function StoreProductPanel({ o2oContext, onClose }: { o2oContext: O2OContextResponse | null, onClose: () => void }) {
  ...
  const handleLock = async (product_id: number) => {
    ...
    try {
      ...
      toast.success("Đã giữ hàng thành công! Vui lòng vào trang Giỏ hàng để thanh toán.")
    } catch (e) {
  ...
  const handleAskAgent = (storeName: string) => {
    ...
    document.dispatchEvent(new CustomEvent('open-agent-chat', { 
      detail: { message: `Hãy tư vấn cho tôi các món quà lưu niệm tại ${storeName}` }
    }))
  }
  ...
  return (
    <div className="absolute top-4 right-4 z-[1000] w-[420px] max-h-[calc(100vh-2rem)] flex flex-col m-4 bg-black/40 backdrop-blur-md border border-white/10 shadow-2xl rounded-2xl overflow-hidden transition-all animate-in fade-in slide-in-from-right-8 duration-300">
      <div className="p-5 border-b border-white/5 bg-gradient-to-l from-purple-500/20 to-transparent flex justify-between items-center">
        <div>
          <h2 className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 truncate w-64 leading-tight">{place_info.name}</h2>
          <p className="text-[10px] text-zinc-400 font-mono mt-1 uppercase tracking-widest"><ShoppingBag className="w-3 h-3 inline mr-1 mb-0.5 text-purple-400"/>O2O Shopping Hub</p>
        </div>
        <button onClick={onClose} className="text-zinc-500 hover:text-white transition-colors bg-white/5 hover:bg-white/10 border border-white/10 p-2 rounded-full"><X className="w-4 h-4" /></button>
      </div>

      <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
        {nearby_stores.length === 0 ? (
          <p className="text-xs text-zinc-500 text-center py-6 bg-white/5 rounded-2xl border border-white/5 font-mono">Chưa có đối tác O2O lân cận.</p>
        ) : (
          <div className="space-y-8">
            {nearby_stores.map(store => (
              <div key={store.store_id} id={`store-${store.store_id}`} className="space-y-3 transition-colors duration-500 p-2 -mx-2 rounded-xl">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-bold text-white flex items-center gap-1.5"><Store className="w-4 h-4 text-cyan-400"/> {store.name}</h3>
                    <p className="text-[10px] text-zinc-400 font-mono uppercase tracking-widest mt-0.5">{store.category || "Retail"}</p>
                  </div>
                  <button onClick={() => handleAskAgent(store.name)} className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 px-3 py-1.5 rounded-full text-[10px] font-mono flex items-center gap-1.5 transition-colors">
                    <Bot className="w-3 h-3"/> Hỏi AI
                  </button>
                </div>
                
                {store.products.length === 0 ? (
                  <p className="text-[10px] text-zinc-500 font-mono">Đang cập nhật sản phẩm...</p>
                ) : (
                  <div className="flex flex-col gap-3">
                    {store.products.map(p => (
                      <div key={p.product_id} className="bg-white/5 border border-white/5 rounded-xl overflow-hidden group hover:bg-white/10 transition-colors flex flex-row items-center p-2 gap-3">
                        <div className="w-16 h-16 rounded-lg overflow-hidden bg-zinc-900 relative shrink-0 border border-white/5 shadow-inner">
                           <img src={p.image_url || "https://via.placeholder.com/150"} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                        </div>
                        <div className="flex-1 flex flex-col justify-between py-0.5">
                          <p className="text-[11px] text-zinc-200 line-clamp-1 leading-relaxed font-medium">{p.name}</p>
                          <p className="text-[10px] font-bold text-emerald-400 font-mono my-1 mb-2">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(p.price)}</p>
                          <button 
                          ...
                          >
                            {lockingProduct === p.product_id ? <Loader2 className="w-3 h-3 animate-spin" /> : "Giữ Hàng"}
                          </button>
                        </div>
                      </div>
                    ))}
...
function PanelOmnisearch({ onSelect, mapRef, userLat, userLon }: { onSelect: (place: any) => void, mapRef: React.MutableRefObject<any>, userLat: number, userLon: number }) {
    ...
    <div className="relative">
      <div className="relative">
        <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
          {isSearching ? <Loader2 className="w-4 h-4 text-emerald-400 animate-spin" /> : <Search className="w-4 h-4 text-zinc-400" />}
        </div>
        <input
          type="text"
          value={query}
          ...
          {results.map((p: any, idx: number) => (
            <div
              ...
              onClick={() => {
                ...
                setQuery(p.name);
                if (mapRef.current) {
                  mapRef.current.flyTo([p.lat, p.lon], 17, { animate: true, duration: 1.5 });
                }
                onSelect(p);
              }}
            ...
            >
              <span className="shrink-0 w-6 h-6 rounded-full bg-gradient-to-br from-cyan-500 to-purple-500 flex items-center justify-center text-[10px] font-bold text-white">{idx + 1}</span>
              <div className="flex-1 min-w-0">
                <div className="font-medium text-emerald-400 truncate text-xs">{p.name}</div>
                <div className="text-[10px] text-zinc-500 truncate mt-0.5">
                  {p.distance_meters != null && <span className="text-cyan-400 font-mono">{p.distance_meters < 1000 ? `${Math.round(p.distance_meters)}m` : `${(p.distance_meters/1000).toFixed(1)}km`}</span>}
                  {p.rating != null && <span className="ml-1.5">⭐ {p.rating}</span>}
...
function AutoFitNearby({ nearbyData }: { nearbyData: any }) {
  const map = useMap()
  useEffect(() => {
    if (nearbyData && nearbyData.places && nearbyData.places.length > 0) {
...
function AutoFitRoute({ routePolyline }: { routePolyline: [number, number][] }) {
  const map = useMap()
  useEffect(() => {
    if (routePolyline && routePolyline.length > 0) {
      map.fitBounds(routePolyline as any, { padding: [50, 50], animate: true, duration: 1.5 })
    }
  }, [routePolyline, map])
  return null
}

function simplifyDouglasPeucker(points: [number, number][], epsilon: number): [number, number][] {
  ...
  const sqrDist = (p: [number, number], p1: [number, number], p2: [number, number]) => {
...
function SpatialOperations() {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      const parsedRad = parseInt(inputRadius)
      if (!isNaN(parsedLat)) setLat(parsedLat)
      if (!isNaN(parsedLon)) setLon(parsedLon)
      if (!isNaN(parsedRad)) setRadius(parsedRad)
    }, 300)
    return () => clearTimeout(handler)
  ...
  const [highlightedPlaceId, setHighlightedPlaceId] = useState<number | string | null>(null)
  ...
  const routePolyline: [number, number][] = useMemo(() => {
    ...
    if (poly && poly.coordinates) {
       const rawCoords = poly.coordinates.map((coord: [number, number]) => [coord[1], coord[0]] as [number, number])
  ...
  const clusterRectangles = useMemo(() => {
    ...
    return clusterData.clusters.map((cluster: any, i: number) => {
      ...
      return (
        <Rectangle 
          key={`cluster-${i}`}
          bounds={[[minLat - latPad, minLon - lonPad], [maxLat + latPad, maxLon + lonPad]]}
          pathOptions={{ color, weight: 2, dashArray: "5, 5", fillColor: color, fillOpacity: 0.1 }}
        />
      )
    })
  }, [clusterData])
        ...
        >
          <Popup>
            <div className="text-xs font-mono p-1">
              ...
              <span className="text-zinc-500 uppercase tracking-widest text-[10px]">{store.category || "Retail"}</span>
            </div>
          </Popup>
        </Marker>
      )
          ...
          <div className="text-xs font-mono p-1 min-w-[150px]">
            ...
            {!routeData && (
              <div className="flex gap-2 mt-3">
                ...
                >
                  {isSelected ? <><Minus className="w-3 h-3" /> Bỏ chọn lộ trình</> : <><Plus className="w-3 h-3" /> Thêm vào lộ trình</>}
                </button>
              </div>
            )}
          </div>
  ...
  }, [nearbyData, selectedNodes, routeData, toggleNodeSelection]);

  return (
    <div className="relative w-full h-[calc(100vh-4rem)] lg:h-screen overflow-hidden bg-zinc-950">
      
      <StoreProductPanel o2oContext={o2oContext} onClose={() => setO2OContext(null)} />
      ...
      <div className="absolute top-4 left-4 z-[1000] w-[380px] max-h-[calc(100vh-2rem)] flex flex-col bg-zinc-950/70 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden" style={{ willChange: "transform, opacity" }}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {nearbyData && nearbyData.places.length > 0 && !routeData && (
              ...
              >
                {isLoadingClusters ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                {isLoadingClusters ? "Analyzing..." : "✨ Phân Tích Cụm (K-Means)"}
              </button>
            )}
          </div>
            ...
            <div className="p-4 bg-purple-500/5 border border-purple-500/20 rounded-2xl space-y-3 animate-in slide-in-from-bottom-2">
              ...
              {routeData && (
                <div className="mt-4 pt-4 border-t border-purple-500/20">
                  ...
                  {routeData.weather_context && (routeData.weather_context as any).condition !== "Unknown" && (
                    <div className="flex items-center justify-center gap-2 text-xs text-amber-400 font-mono bg-amber-500/10 p-2 rounded-lg border border-amber-500/20">
                      ...
                      Thời tiết: {(routeData.weather_context as any).condition} ({(routeData.weather_context as any).temperature}°C)
                    </div>
                  )}
                </div>
              )}
